...
```{r lunch_type and sex}
#TODO: change colors resp ask someone!
...
#stats influence of interventions => somehow super slow
sell_ind_dat_2017 %>% 
  filter(ccrs %in% df$ccrs) %>%
...
dat <- sell_ind_dat_2017 %>% 
  ...
  mutate(gender = recode(gender,
         ...
         type = case_when(lunch_type == "buffet" |
                          ...
                          lunch_type == "veg-flexitarian" |
                          lunch_type == "meat-flexitarian" ~ "vegi", 
                          TRUE ~ "meat")) #TODO: check::attention why 21040 meal sellings
  
#little stats change in meat purchase per condit
dat %>% 
  group_by(label_content, type, condit) %>% 
  summarise(tot = n()) %>% 
  mutate(pct = tot / sum (tot)) %>% 
  ungroup() %>% 
  group_by(type, condit) %>%
  mutate(pct_tot = pct / sum(pct)) %>% 
  ungroup()  -> df_tot
...
for (i in 1:length(dfList)) {
  ...
  p <- ggplot(data = pl,
            aes(x = forcats::fct_rev(xlab),
                ...
                color = label_color)) + 
    geom_bar(stat = "identity", position = position_stack(), color = NA) + # set color NA otherwise error occurs
    xlab("") +
...
#add cols
Cols = c("buffet" = "#5D6395", 
         "never meat" = "#1E88E5",
         "veg-flexitarian" = "#FFC107",
         "meat-flexitarian" = "#004D40",
         "meat-eater" = "#AAED89" , 
         "meat lover" =  "#F59D28",
         "always meat" = "#D81B60") # add color

#add color for label:not working
dat$label_color <- as.factor(sapply(unlist(Cols)[dat$lunch_type], 
                                         function(color) { if (isDark(color))
                                           'white' else 'black' })) 
#plot
p <- ggplot(data = dat, aes(area = count, 
                ...
                label = label)) +
                # color = label_color)) +
  treemapify::geom_treemap(color = NA) + # use layout = fiexed for another order, however is not nice
...
dat <- sell_overall %>% 
  ...
  select(-tot_sold) %>% 
  left_join(sell_overall, .[ , c("sex", "condit", "text")], 
            by = c("gender", "condit")) %>% 
...
p <- ggplot(data = dat,
    ...
    scale_fill_manual(values = ColsperContent,
                      breaks = attributes(ColsperContent)$name)+
    scale_color_manual(values = levels(dat$label_color)) +
    geom_text(aes(label=ifelse(pct > .015, scales::percent(round(pct, 2),
                                                           accuracy = 1), "")),
              size = 22 * converter, 
              position = position_stack(vjust = 0.52),
              fontface = "bold")+
...
#set colors for plot
ColsPerVisit = c(
"6 to 11 times" = "#99f200",
"12 to 23 times" =  "#008099",
"24 to 35 times" = "#fad60d",
"36 to 47 times" = "#c5b87c",
"48 to 60 times" = "#262626")


dat$label_color <- as.factor(sapply(unlist(ColsPerVisit)[dat$category], 
                                         function(color)
                                           { if (isDark(color))
                                             'white' else 'black' })) 
#plot data
p <- ggplot(dat, aes(x = xlab, 
                     y = pct, 
                     fill = category, 
                     color = label_color)) +
    geom_bar(stat = "identity", colour = NA, 
             position = position_stack(reverse = TRUE), 
             width = .7) + 
    scale_fill_manual(breaks = attributes(ColsPerVisit)$name,
                      limits = attributes(ColsPerVisit)$name,
                      values =  ColsPerVisit) +
    scale_color_manual(values = levels(dat$label_color)) +
    coord_flip() +
    ...
    scale_x_discrete(limits = rev(c("buffet\nn=19",
                     ...
                     labels = rev(c("Buffetarian\n (19)",
                                    ...
                                    " Always Meat\n(18)"))) +
    xlab("") +
    ylab("\nCanteen visit frequency during 12 weeks\n") +
    geom_text(aes(label = ifelse(pct < .02,"", 
                                 scales::percent(round(pct, 2), accuracy = 1L))), 
              position = position_stack(vjust = .5, reverse = TRUE), 
              size= 22 * converter,
              fontface = "bold") +
    mytheme_facet 
...
#save as pdf
ggsave(filename = paste0(wd_plots, "/lunch_type_frequency_",
                         format(Sys.Date(), "%y%m%d"),
                         ".pdf"),
       p,
       height = 8,
...
```{r another approach for lunch_type and experiment, eval=FALSE}
