...
#load packages
pacman::p_load(dplyr, forcats, readr, ggplot2, treemap)
...
#### response to intervention: stats
...
####  stats response to vegan dishes

```{r response to vegan dishes}
...
##Attention: original plot from here (htps://novanimal.ch/wp-content/uploads/2020/11/2020_ZHAW_egeler_baur_menuwahl_NOVANIMAL.pdf#page=51) could not be reproduced
...
dat <- reg_visitors %>% 
    ...
    ungroup() %>% 
    mutate(info_type = recode(lunch_type, 
                             "buffet" = "",
                             "never meat" = "",
                             "veg-flexitarian" = "Meat share: < 1/4", 
                             "meat-flexitarian" = "Meat share: \u2265 1/4 to < 1/2",
                             "meat-eater" = "Meat share: \u2265 1/2 to < 3/4", 
    ...
    mutate(label_ = paste(lunch_type2, paste(round(pct*100, 1), "%", sep = " "), sep = "\n"),
           label = paste(label_, info_type, sep = "\n")) %>% 
    mutate(color = recode(lunch_type, 
                      "buffet" = "#5D6395", 
                      "never meat" = "#1E88E5",
...
cairo_pdf(filename = paste0(wd_plots, "/map_lunch_type_",
                         format(Sys.Date(), "%y%m%d"),
                         ".pdf"),
          width = 20, height = 20)


# order is different than in published one
treemap::treemap(dat, #Your data frame object
        ...
        # sortID = "count", # somehow not working
        algorithm = "pivotSize",
        sortID = "test2",
