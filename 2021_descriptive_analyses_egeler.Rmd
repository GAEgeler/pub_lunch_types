...
sell_dat_2017 %>% 
  filter(ccrs %in% df$ccrs) %>%
  filter(member != "Spezialkarten") %>% 
  left_join(., df[, c("ccrs", "food_type")], by = "ccrs") -> df_interv
...
dat <- visiter  %>% 
    ...
    summarise(tot = sum(visit_counts)) %>% 
    ungroup() %>% 
    left_join(., visiter, by = "food_type") %>% 
    mutate(label = paste("n", tot, sep = "="),
           xlab = paste(food_type, label, sep = "\n")) 
#set colors for plot
ColsPerVisit = c(
...
p <- ggplot(dat, aes(x = xlab, 
    ...
    scale_x_discrete(limits = rev(c("buffet\nn=19",
                     ...
                     labels = rev(c("Buffetarian\n (19)",
                                    ...
                                    "Always Meat\n(18)"))) +
    xlab("") +
    ylab("\nVisit frequency during 12 weeks\n") +
    geom_text(aes(label = ifelse(visiter2$pct<.02,"", 
...
#save as pdf
ggsave(filename = paste0(wd_plots, "/food_type_frequency_",
                         format(Sys.Date(), "%y%m%d"),
                         ".pdf"),
       p,
       height = 6.5,
