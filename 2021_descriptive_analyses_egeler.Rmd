...
# prepare data overall: exclude one case with missing gender
dat <- reg_visitors %>% 
  group_by(food_type) %>% 
  summarise(visit_counts=n()) %>% 
  mutate(pct=visit_counts/sum(visit_counts)) %>% 
...
ggsave(filename = paste0(wd_plots, "/food_type_gender_", format(Sys.Date(), 
                                                                "%y%m%d") 
                         ,".pdf"), p, 
       height = 4,
       width = 22,
...
p <- ggplot(dat, aes(y = pct_gender, 
  ...
  geom_text(aes(label=ifelse(pct_gender<0.02,"", 
                             scales::percent(round(pct_gender, 2),
                                             accuracy = 1))), 
            size = 22 * converter, position = position_stack(vjust = 0.5, reverse = TRUE)) + 
...
#TODO: change order of xaxis - for double axis => probably with interaction
#TODO: delete some of the items in the legends
...
```{r another approach for food_type and experiment}
# buffetarians and never meat go together; meat_lovers and alwys_meat too 
visiter <- reg_visitors %>%
...
# my colors for the plot
Cols = c("Unknown" = "black","Vegan" = "#80ccff", "Vegetarian" = "#c5b87c", "Fish"="#6619e6", "Meat" = "#fad60d", "Hot and Cold"="#4c4848")
